		...
		const u8 *ttconfig_data, int ttconfig_size)
{
	...
	rc = cyttsp4_get_ttconfig_crc(ttsp, ttconfig_data, ttconfig_size,
      ...
      if (rc)
		return 0;
    /*Get config version from platform*/
	cyttsp4_get_ttconfig_version_from_platform(ttsp, ttconfig_data, &cfg_ver_new);
	if (!data->si)
		return 0;

	dev_dbg(dev, "%s: img_crc:0x%04X new_crc:0x%04X img_cfg:0x%04X new_cfg:0x%0X\n",__func__, data->si->ttconfig.crc, cfg_crc_new,data->si->ttconfig.version,cfg_ver_new);
    if (cfg_ver_new > (data->si->ttconfig.version & 0xFF))  // new config versio high
    {
       dev_dbg(dev, "%s: ttconfig is newer, will upgrade\n",__func__);
       return 1;
    }
    else if (cfg_ver_new == (data->si->ttconfig.version & 0xFF))  // new config version equal old config version
    {
        if (cfg_crc_new != data->si->ttconfig.crc)  // crc not equql
        {
    ...
    {
        dev_dbg(dev, "%s: Config Version is low ,will NOT upgrade\n", __func__);
        return 0;
    }
}
